...
for file in files:
    ...
    for imp in header:
        ...
        if (not imp_file.startswith("Mathlib.")) and \
            (not imp_file.startswith("MiscYD.Mathlib")):
            raise OSError(f"""
MathlibImportError: A file in MiscYD.Mathlib imports a file not in Mathlib or MiscYD.Mathlib
\tImporting file: {file}
